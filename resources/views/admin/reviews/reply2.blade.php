            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <section class="">
                    <div class="flex flex-row items-center justify-evenly border">
                        <div class="p-2 capitalize">Sender ({{ $previews->user->name }})</div>
                        <div class="p-2 capitalize">Receipent ({{ Auth::user()->name }})</div>
                    </div>
                    <div class="border">
                        <div class="flex items-center justify-start p-5 pt-5">
                            <span class="pr-3 capitalize">{{ $previews->user->name }} :</span>
                            ...
                            <span class="rounded-xl border px-2 py-3">
                                @php
                                    ...
                                    foreach ($replies as $reply) {
                                        if ($reply->preview_id == $previews->id) {
                    ...
                    <div class="flex">
                        <form class="flex w-full" method="POST" action="{{ route('admin.reviews.submitReply2') }}">
                            @csrf
                            <input type="hidden" name="review_id" value="">
                            <input type="hidden" name="preview_id" value="{{ $previews->id }}">
                            {{-- @foreach ($replies as $reply) --}}
                                @if (!$finalReply)
